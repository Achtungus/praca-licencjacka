...
public class MCTSNode
{
    ...
    int after40bonus = 30;
    public double Heuristic(SeededGameState gameState, TierListEv cardTierList)
    {
        ...
        int cardsValues = GlobalStats.strategy.CardsValues(gameState, cardTierList, GlobalStats.cardsCount);
    ...
    public (double, SeededGameState) Simulate(MCTSNode node, TierListEv cardTierList, SeededRandom rng)
    {
        if (node.endTurn || node.children.Count == 1)
        {
            return (Heuristic(node.gameState, cardTierList), node.gameState);
        }
        ...
        return (Heuristic(gameState, cardTierList), gameState);
    }
}
...
public class BestMCTS : AI
{
    ...
    public BestMCTS(TierListEv cardTierList)
    {
    ...
    private (double, SeededGameState) run(MCTSNode node, SeededRandom rng)
    {
        ...
        {
            var (score, simulatedGameState) = node.Simulate(node, cardTierList, rng);
            node.visits += 1;
            node.score = score;

...
public class CardTierEv
{
    ...
    {
        return $"{Name} {Deck} {Tier[0]} {Tier[1]} {Tier[2]}";
    }
...
public class TierListEv
{
    ...
    {
        string res = "";
        foreach (CardTierEv card in cardTierList)
        {
            res += card.ToString() + "\n";
        }
        return res;
    ...
    public static TierListEv Combine(List<TierListEv> parents)
    {
        ...
        List<int> patronToParent = new();
        for (int i = 0; i < maxPatronId + 1; i++)
        {
            patronToParent.Add(rnd.Next(0, parents.Count));
        }

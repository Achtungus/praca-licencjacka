...
public class NodeBug
{
    ...
    public List<NodeBug>? childs;
    public NodeBug? father;
    ...
    public NodeBug(SeededGameState fatherGameState, Move? nodeMove, NodeBug? fatherOrig, List<Move> possibleMoves = null)
    {
        this.wins = 0;
        this.visits = 0;
    ...
    public void CreateChilds()
    {
        ...
        {
            this.childs.Add(new NodeBug(this.nodeGameState, childMove, this));
        }
    }
    ...
    public NodeBug SelectBestChild()
    {
        NodeBug bestChild = this.childs[0];

        double bestScore = 0;

        foreach (NodeBug child in this.childs)
        {
    ...
    public void UpdateAllPossibleMoves()
    {
        ...
        foreach (Move possibleMove in this.possibleMoves)
        {
            ...
            {
                this.childs.Add(new NodeBug(this.nodeGameState, possibleMove, this));
            }
        }
...
public class MCTSBotBug : AI
{
    ...
    private NodeBug TreePolicy(NodeBug v, SeededRandom rng)
    {
        if (v.childs.Count() > 0)
        {
            ...
            int selectedChild = 0;
            int index = 0;
            foreach (NodeBug childNode in v.childs)
            {
    ...
    private void BackUp(NodeBug? v, double delta)
    {
        if (v is not null)
        {
            v.visits += 1;
    ...
    public override Move Play(GameState gameState, List<Move> possibleMoves)
    {
        ...
        if (startOfTurn)
        {
            actRoot = new NodeBug(sgs, null, null, possibleMoves);

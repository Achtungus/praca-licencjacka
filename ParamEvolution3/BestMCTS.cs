...
public class BestMCTS : AI
{
    ...
    void SelectStrategy(GameState gameState)
    {
        ...
        {
            strategy = new GameStrategy(cardCount, GamePhase.LateGame, gameParams);
        }
        else if (points <= 10 && gameState.EnemyPlayer.Prestige <= 13)
        {
            strategy = new GameStrategy(cardCount, GamePhase.EarlyGame, gameParams);
        }
        else
        {
            strategy = new GameStrategy(cardCount, GamePhase.MidGame, gameParams);
        }
    }
    ...
    public override Move Play(GameState gameState, List<Move> possibleMoves)
    {
        ...
        if (startOfTurn || !CheckIfSameGameStateAfterOneMove(root, gameState))
        {
            ...
            root = new MCTSNode(seededGameState, filteredMoves);
        }
        ...
        if (move_out.Command == CommandEnum.END_TURN)
        {
            startOfTurn = true;
        }
        return move_out;
